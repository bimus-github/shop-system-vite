  ...
  return useMutation({
    mutationFn: async (product: Saled_Product_Type) => {
      ...
      if (result === Message_Forms.SUCCESS) {
        queryClient.setQueryData(['productsInSaleRoom'], (prevProducts: Saled_Product_Type[]) => [
          ...prevProducts,
          product
        ])
        return result
...
export const useDeleteProductFromRoom = (roomId: string) => {
  ...
  return useMutation({
    mutationFn: async (productId: string) => {
      const result: Message_Forms = await deleteProductFromRoom(roomId, productId)

      if (result === Message_Forms.SUCCESS) {
        queryClient.setQueryData(['productsInSaleRoom'], (prevProducts: Saled_Product_Type[]) => {
          return prevProducts.filter((p: Saled_Product_Type) => p.id !== productId)
        })
      } else {
        return result
      }
