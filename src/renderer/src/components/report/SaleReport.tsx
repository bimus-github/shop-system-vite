...
const Body = ({ start, end }: { start: Dayjs; end: Dayjs }) => {
  ...
  const saledProducts = useMemo(() => {
    return allData?.filter(
      (p) =>
        dayjs(new Date(p.saled_date)).isAfter(start) &&
        dayjs(new Date(p.saled_date)).isBefore(end)
    );
  }, [allData, start, end]);
    ...
    <TableRow>
      ...
      <TableCell>
        {saledProducts
          ?.reduce(
            (a, b) =>
              a + b.saled_count * b.saled_price * (1 - b.discount / 100),
            0
          )
          .toLocaleString() || 0}{" "}
        {langFormat({ uzb: "so'm", en: "so'm", ru: "сум" })}
      ...
      <TableCell>
        {saledProducts
          ?.reduce(
            (a, b) =>
              a +
              b.saled_count * (b.saled_price * (1 - b.discount / 100) - b.cost),
            0
          )
          .toLocaleString() || 0}{" "}
        {langFormat({ uzb: "so'm", en: "so'm", ru: "сум" })}
...
const CustomTtitle = ({
  ...
  return (
    <FormControl
      variant="standard"
      sx={{ display: "flex", flexDirection: "row", gap: 1, ml: 2 }}
    >
      <DatePicker
        label={langFormat({
          uzb: "...dan",
        ...
        value={start}
        onChange={(newStart) => {
          localStorage.setItem("start-sale", newStart.format("DD/MM/YYYY"));
          setStart(newStart);
        }}
        sx={{ mr: 1, width: "200px" }}
      />
      <DatePicker
        ...
        value={end}
        onChange={(newEnd) => {
          localStorage.setItem("end-sale", newEnd.format("DD/MM/YYYY"));
          setEnd(newEnd);
        }}
        sx={{ mr: 1, width: "200px" }}
      />
...
function SaleReport() {
  ...
  return (
    <CustomTable
      ...
      tablebody={<Body start={start} end={end} />}
      tablehead={<Head />}
      customTitle={
        <CustomTtitle
          start={start}
